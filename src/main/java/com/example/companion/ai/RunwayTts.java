...
public final class RunwayTts {
    ...
    /** Make text read better aloud: "2x2" -> "2 by 2"; a coordinate triple becomes
     *  "coordinates" + each number read digit-by-digit, with a minus read as "negative". */
    private static String speakable(String text) {
        ...
        // Normalize any labels/words the model may add so coord detection works:
        t = t.replaceAll("(?i)\\bnegative\\s+(?=\\d)", "-"); // "negative 918" -> "-918"
        t = t.replaceAll("(?i)\\b[xyz]\\s+(?=-?\\d)", "");    // "x -918" -> "-918", "y 107" -> "107"

        // Coordinate triples -> "coordinates <digits>, <digits>, <digits>"
        ...
        while (m.find()) {
            ...
            String rep = coordLike
                ? "coordinates " + readDigits(g1) + ", " + readDigits(g2) + ", " + readDigits(g3)
        ...
        // Any remaining standalone negative number -> "negative 500" (natural for distances etc.)
        t = t.replaceAll("(?<![\\w\\d])-(\\d)", "negative $1");
        return t;
    }

    /** Read a (possibly negative) integer digit-by-digit: "-918" -> "negative nine one eight". */
    private static String readDigits(String num) {
        boolean neg = num.startsWith("-");
        String d = neg ? num.substring(1) : num;
        String[] w = {"zero","one","two","three","four","five","six","seven","eight","nine"};
        StringBuilder b = new StringBuilder();
        if (neg) b.append("negative");
